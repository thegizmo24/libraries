...
class Paypal {
	...
	function __initialize($props = array())
	{
		...
		$config["currency_code"] = ''; //http://bit.ly/anciiH
		$config["disp_tot"] = 'Y';
	...
	function pay()
	{
		...
		if ($this->config['production'] == TRUE) header('LOCATION:'.$this->production_url.$vars);
		else header('LOCATION:'.$this->sandbox_url.$vars);
	}
